...
public class SensorInstanceService : ISensorInstanceService
{
    ...
    public async Task<bool> UpdateAsync(SensorInstance sensorInstance, CancellationToken cancellationToken)
    {
        ...
        {
            var target;
            target = await _ioTDevices.GetByIdAsync(sensorInstance.IoTDeviceId, cancellationToken)
                ?? throw new InvalidOperationException("IoTDevice not found.");

            if (target.SensorInstance is not null && target.SensorInstance.Id != existing.Id)
            {
